...
#if !defined(MAILIO_ENABLE_EXPERIMENTAL_POOL)
#error "mailio::pool::imap_pool is experimental and depends on legacy IMAP. Define MAILIO_ENABLE_EXPERIMENTAL_POOL to enable it (legacy IMAP required)."
#else
...
#include <mailio/pool/connection_pool.hpp>
#include <mailio/pool/pool_config.hpp>
#include <mailio/imap/client.hpp>
#include <mailio/net/upgradable_stream.hpp>

namespace mailio::pool
{
...
public:
    ...
    using stream_type = ssl::stream<mailio::net::upgradable_stream>;
    using client_type = mailio::imap_client<stream_type>;
    ...
    void configure(
        ...
        mailio::imap_base::auth_method_t auth_method = mailio::imap_base::auth_method_t::LOGIN)
    {
     ...
     */
    awaitable<std::pair<pooled_type, mailio::imap_base::mailbox_stat_t>> 
     ...
     */
    awaitable<mailio::imap_base::response_t> fetch(
    ...
    awaitable<std::vector<unsigned long>> search(
        const std::string& mailbox,
        const std::list<mailio::imap_base::search_condition_t>& conditions,
        bool uid = false)
    {
        auto conn = co_await acquire();
...
private:
    ...
    awaitable<std::unique_ptr<client_type>> create_client()
    {
        ...
        mailio::net::upgradable_stream base_stream(executor_);
        ...
        mailio::net::dialog<mailio::net::upgradable_stream> dlg(std::move(base_stream));
    ...
    mailio::imap_base::auth_method_t auth_method_ = mailio::imap_base::auth_method_t::LOGIN;
...
#endif // MAILIO_ENABLE_EXPERIMENTAL_POOL
