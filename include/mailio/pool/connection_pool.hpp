...
public:
    ...
    {
        MAILIO_LOG_INFO("POOL", "Warming up pool with " << config_.min_connections << " connections");
    ...
    {
        MAILIO_LOG_INFO("POOL", "Draining pool...");
        shutdown_.store(true, std::memory_order_release);
        ...
        MAILIO_LOG_INFO("POOL", "Pool drained");
    }
...
private:
    ...
    awaitable<pooled_type> acquire_impl()
    {
        ...
        while (true)
        {
            ...
            if (client)
            {
                ...
                {
                    should_recycle = true;
                    MAILIO_LOG_DEBUG("POOL", "Connection exceeded max lifetime, recycling");
                }
                ...
                if (!should_recycle && config_.validate_on_acquire && validator_)
                {
                    try
                    {
                        ...
                        {
                            should_recycle = true;
                            MAILIO_LOG_DEBUG("POOL", "Connection validation failed, recycling");
                        }
                    }
                    catch (...)
                    {
                        should_recycle = true;
                        MAILIO_LOG_DEBUG("POOL", "Connection validation threw exception, recycling");
                    }
                }
            ...
            if (can_create)
            {
                ...
                {
                    MAILIO_LOG_WARN("POOL", "Failed to create connection: " << e.what());
                    {
                        std::lock_guard lock(stats_mutex_);
            ...
            MAILIO_LOG_DEBUG("POOL", "Pool at capacity, waiting for connection...");
    ...
    awaitable<std::unique_ptr<Client>> create_connection()
    {
        ...
        while (attempts < config_.creation_retry_count)
        {
            ...
            catch (...)
            {
                ...
                {
                    MAILIO_LOG_DEBUG("POOL", "Connection creation failed, retry " 
    ...
    awaitable<void> create_and_add_connection()
    {
        ...
        {
            MAILIO_LOG_WARN("POOL", "Failed to create warmup connection: " << e.what());
            {
                std::lock_guard lock(stats_mutex_);
                ++stats_.connections_failed;
            }
