...
{
    mailio::detail::ensure_no_crlf_or_nul(text, "astring");
    std::string escaped = mailio::codec::escape_string(std::string(text), "\"\\");
    return mailio::codec::surround_string(escaped);
}

[[nodiscard]] inline std::string to_mailbox(std::string_view utf8_mailbox)
{
    return to_astring(mailio::encode_modified_utf7(utf8_mailbox));
}
...
namespace detail
{
        ...
        std::string_view flags, std::string_view date_time, bool literal_plus)
    {
        ...
        mailio::detail::ensure_no_crlf_or_nul(flags, "flags");
        mailio::detail::ensure_no_crlf_or_nul(date_time, "date_time");

        const std::string mailbox_q = mailio::imap::to_mailbox(mailbox);
        std::string cmd;
        mailio::detail::append_sv(cmd, "APPEND");
        mailio::detail::append_space(cmd);
        mailio::detail::append_sv(cmd, mailbox_q);
        if (!flags.empty())
        {
            mailio::detail::append_space(cmd);
            mailio::detail::append_sv(cmd, flags);
        }
        if (!date_time.empty())
        {
            const std::string dt = mailio::imap::to_astring(date_time);
            mailio::detail::append_space(cmd);
            mailio::detail::append_sv(cmd, dt);
        }
        mailio::detail::append_space(cmd);
        mailio::detail::append_char(cmd, '{');
        mailio::detail::append_uint(cmd, static_cast<std::uint64_t>(size));
        if (literal_plus)
            mailio::detail::append_char(cmd, '+');
        mailio::detail::append_char(cmd, '}');
        return cmd;
...
struct options
{
    ...
    mailio::net::tls_options tls;
    mailio::net::tls_mode default_tls_mode = mailio::net::tls_mode::none;
