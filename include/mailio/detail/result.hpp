...
No exceptions are thrown in mailio - all errors are returned via result<T>.
...
/// Error categories for mailio operations
...
/// Usage: auto val = MAILIO_CO_TRY(co_await some_async_op());
#define MAILIO_CO_TRY(expr) \
    ({ \
        auto&& _result = (expr); \
        if (!_result) [[unlikely]] \
...
#define MAILIO_CO_TRY_VOID(expr) \
    do { \
        auto&& _result = (expr); \
        if (!_result) [[unlikely]] \
            co_return std::unexpected(std::move(_result).error()); \
