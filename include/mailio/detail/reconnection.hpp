...
public:
    ...
    auto with_reconnection(ConnectFn connect_fn, OperationFn operation_fn)
        -> mailio::asio::awaitable<decltype(std::declval<OperationFn>()().get())>
    {
        ...
        while (true)
        {
            ...
            catch (const std::exception& e)
            {
                ...
                MAILIO_LOG_INFO("RECONNECT", "Attempt " << attempt << " in " << delay.count() << "ms");
                ...
                {
                    MAILIO_LOG_WARN("RECONNECT", "Connection failed: " << conn_error.what());
