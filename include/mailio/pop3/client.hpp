...
private:
    struct session_snapshot
    {
        ...
        std::string host;
        std::string service;
        mailio::net::tls_mode tls_mode{mailio::net::tls_mode::none};
...
public:
    ...
    explicit client(any_io_executor executor, options opts = {})
        : options_(std::move(opts)),
          dlg_(mailio::net::upgradable_stream(executor), options_.max_line_length, options_.timeout),
    ...
    awaitable<void> connect(const std::string& host, uint16_t port, mailio::net::tls_mode mode,
    ...
    awaitable<mailio::message> retr_message(unsigned message_no)
    {
        [[maybe_unused]] auto guard = co_await mutex_.lock();
...
private:
    ...
    static void ensure_reconnect_ready(const session_snapshot& snapshot)
    {
        ...
        if (snapshot.tls_mode != mailio::net::tls_mode::none && snapshot.tls_ctx == nullptr)
    ...
    void remember_tls(mailio::net::tls_mode mode, ssl::context* tls_ctx, std::string_view sni)
    {
    ...
    awaitable<void> reconnect_with_snapshot(const session_snapshot& snapshot)
    {
        ...
        if (snapshot.tls_mode == mailio::net::tls_mode::starttls && !options_.auto_starttls)
        {
        ...
        ssl::context* tls_ctx = nullptr, std::string sni = {})
    {
        ...
        mailio::net::upgradable_stream stream(executor);
        ...
        if (mode == mailio::net::tls_mode::implicit)
        {
            ...
            remember_tls(mailio::net::tls_mode::implicit, tls_ctx, resolved_sni);
    ...
    awaitable<void> start_tls_impl(ssl::context& context, std::string sni)
    {
        ...
        remember_tls(mailio::net::tls_mode::starttls, &context, resolved_sni);
    ...
    awaitable<void> apop_impl(const std::string& username, const std::string& password)
    {
        ...
        std::string cmd;
        mailio::detail::append_sv(cmd, "APOP ");
        mailio::detail::append_sv(cmd, username);
        mailio::detail::append_space(cmd);
        mailio::detail::append_sv(cmd, digest);
    ...
    {
        mailio::detail::ensure_auth_allowed<error>(dlg_.stream().is_tls(), options_);
    }
    ...
    awaitable<message_list_t> list_impl(unsigned message_no)
    {
        ...
        mailio::detail::append_uint(cmd, message_no);
        co_await send_command(cmd);
    ...
    awaitable<uidl_list_t> uidl_impl(unsigned message_no)
    {
        ...
        {
            mailio::detail::append_sv(cmd, "UIDL ");
            mailio::detail::append_uint(cmd, message_no);
        }
        co_await send_command(cmd);
    ...
    awaitable<std::string> retr_impl(unsigned long message_no)
    {
        ...
        std::string cmd;
        mailio::detail::append_sv(cmd, "RETR ");
        mailio::detail::append_uint(cmd, message_no);
        co_await send_command(cmd);
    ...
    awaitable<std::string> top_impl(unsigned message_no, unsigned lines)
    {
        ...
        std::string cmd;
        mailio::detail::append_sv(cmd, "TOP ");
        mailio::detail::append_uint(cmd, message_no);
        mailio::detail::append_space(cmd);
        mailio::detail::append_uint(cmd, lines);
        co_await send_command(cmd);
    ...
    awaitable<mailio::message> retr_message_impl(unsigned message_no)
    {
    ...
    awaitable<void> dele_impl(unsigned long message_no)
    {
        ...
        std::string cmd;
        mailio::detail::append_sv(cmd, "DELE ");
        mailio::detail::append_uint(cmd, message_no);
        co_await send_command(cmd);
