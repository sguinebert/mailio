...
Connection pooling for mailio clients.
...
#include <mailio/pool/smtp_pool.hpp>
// #include <mailio/pool/imap_pool.hpp>  // Requires imap_client refactoring
 ...
 * 
 * @code
 * #include <mailio/pool.hpp>
 * 
 * asio::io_context ctx;
 * ssl::context tls_ctx(ssl::context::tlsv13_client);
 * 
 * mailio::pool::smtp_pool pool(ctx.get_executor());
 ...
 * 
 * @code
 * std::vector<mailio::message> messages;
 * // ... populate messages ...
 * 
 ...
 * auto config = mailio::pool::pool_config::high_traffic();
 * 
 * // Or customize
 * mailio::pool::pool_config config;
 * config.min_connections = 5;
 * config.max_connections = 50;
 * config.idle_timeout = std::chrono::seconds{120};
 * config.validate_on_acquire = false;  // Faster but riskier
 * 
 * mailio::pool::smtp_pool pool(executor, config);
 * @endcode
 * 
