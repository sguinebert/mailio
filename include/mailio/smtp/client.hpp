...
public:
    ...
    awaitable<reply> send(const mailio::message& msg, const envelope& env = envelope{})
    {
        [[maybe_unused]] auto guard = co_await mutex_.lock();
     ...
     */
    awaitable<reply> send(const mailio::message& msg, const envelope_dsn& env)
    {
        [[maybe_unused]] auto guard = co_await mutex_.lock();
     ...
     * @return Server reply
     */
    awaitable<reply> send(const mailio::message& msg, const envelope_ext& env)
    {
        [[maybe_unused]] auto guard = co_await mutex_.lock();
     ...
     */
    awaitable<reply> send_pipelined(const mailio::message& msg, const envelope& env = envelope{})
    {
        [[maybe_unused]] auto guard = co_await mutex_.lock();
...
private:
    ...
    static mail_data_info prepare_mail_data(const mailio::message& msg, const std::string& mail_from,
    ...
    {
        mailio::smtp::detail::mail_extension_flags flags;
        ...
        mailio::smtp::detail::append_mail_extensions(cmd, info.size, info.has_8bit, info.needs_utf8, flags);
    }
    ...
    void trace_payload(std::string_view label, std::size_t bytes) const
    {
        ...
        mailio::detail::append_sv(line, " payload bytes=");
        ...
        logger.trace_protocol("SMTP", mailio::log::direction::send, line);
    }
    ...
    awaitable<reply> ehlo_impl(std::string domain)
    {
        ...
        if (!domain.empty())
            mailio::detail::ensure_no_crlf_or_nul(domain, "domain");
        const std::string helo_name = domain.empty() ? default_hostname() : std::move(domain);
        mailio::detail::ensure_no_crlf_or_nul(helo_name, "helo_name");
    ...
    awaitable<reply> send_impl(const mailio::message& msg, const envelope& env)
    {
        if (!has_greeting())
        ...
        mailio::detail::ensure_no_crlf_or_nul(mail_from, "mail_from");
        ...
        std::string cmd;
        mailio::detail::append_sv(cmd, "MAIL FROM: ");
        mailio::detail::append_angle_addr(cmd, mail_from);
        ...
        for (const auto& rcpt : recipients)
        {
            mailio::detail::ensure_no_crlf_or_nul(rcpt, "rcpt_to");
            cmd.clear();
            mailio::detail::append_sv(cmd, "RCPT TO: ");
            mailio::detail::append_angle_addr(cmd, rcpt);
     ...
     */
    awaitable<reply> send_dsn_impl(const mailio::message& msg, const envelope_dsn& env)
    {
        if (!has_greeting())
            throw error("Send requires a greeting.", "");
        if (!has_helo_or_ehlo())
        ...
        if (dsn_active)
        {
            ...
            {
                mailio::detail::ensure_no_crlf_or_nul(env.dsn.envid, "dsn.envid");
                cmd += " ENVID=";
                cmd += env.dsn.envid;
            }
        }
        ...
        {
            mailio::detail::ensure_no_crlf_or_nul(env.dsn.orcpt, "dsn.orcpt");
     ...
     */
    awaitable<reply> send_ext_impl(const mailio::message& msg, const envelope_ext& env)
    {
        if (!has_greeting())
            throw error("Send requires a greeting.", "");
        if (!has_helo_or_ehlo())
     ...
     */
    awaitable<reply> send_pipelined_impl(const mailio::message& msg, const envelope& env)
    {
        if (!has_greeting())
            throw error("Send requires a greeting.", "");
        if (!has_helo_or_ehlo())
    ...
    {
        mailio::detail::ensure_no_crlf_or_nul(username, "username");
        mailio::detail::ensure_no_crlf_or_nul(access_token, "access_token");
