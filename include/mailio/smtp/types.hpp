...
namespace detail
{
        ...
        const mail_extension_flags& flags)
    {
        ...
        {
            mailio::detail::append_sv(cmd, " SIZE=");
            mailio::detail::append_uint(cmd, static_cast<std::uint64_t>(msg_size));
        }
        if (flags.use_smtputf8 && flags.supports_smtputf8 && needs_utf8)
            mailio::detail::append_sv(cmd, " SMTPUTF8");
        if (flags.use_8bitmime && flags.supports_8bitmime && has_8bit)
            mailio::detail::append_sv(cmd, " BODY=8BITMIME");
    }
