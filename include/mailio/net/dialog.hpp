...
public:
    ...
    **/
    ...
    auto write_line(std::string_view line, CompletionToken&& token)
    {
        ...
        trace_line(mailio::log::direction::send, payload);
    ...
    auto read_line(CompletionToken&& token)
    {
        return asio::async_compose<CompletionToken, void(asio::error_code, std::string)>(
            [this, started = false](auto& self, asio::error_code ec = {}, std::size_t = 0) mutable
            {
                ...
                trace_line(mailio::log::direction::receive, line);
...
protected:
    ...
    void trace_line(mailio::log::direction dir, std::string_view data) const
    {
        auto& logger = mailio::log::logger::instance();
        if (!logger.is_trace_enabled())
            return;
        if (dir == mailio::log::direction::send && redact_secrets_in_trace_)
        {
            logger.trace_protocol(trace_protocol_, dir, mailio::detail::redact_line(data));
            return;
        }
